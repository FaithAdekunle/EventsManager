...
class AdminHome extends React.Component {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        <div
          className="centers-loader success-background"
          ...
          <div className={`row ${this.props.centers ? '' : 'hidden'}`}>
            ...
            <div className="col-md-4">
              <ul className="list-group centers-list">
                {
                  Helpers
                    .sortByName(this.props.centers).map((center, index) => (
                      <li
                        className="list-group-item"
                        key={center.id}
                        onClick={() => this.goToCenter(center.id, index)}
                      >
                        {center.name}
                      </li>
                    ))
